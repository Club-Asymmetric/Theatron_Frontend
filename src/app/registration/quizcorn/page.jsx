...
export default function QuizcornRegistration() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/register/group/quizcorn`, {
        method: 'POST',
        headers: {
            ...
            <form className="space-y-6" onSubmit={handleSubmit}>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={handleInputChange}
                  className="w-full bg-gray-900 border border-gray-700 px-4 py-2 text-white focus:border-red-600 focus:outline-none transition"
                  placeholder="Enter participant 2 college"
                />
              </div>

              <div>
                <label className="block text-sm font-bold mb-2">Participant 3 Name <span className="text-gray-500">(Optional)</span></label>
                <input
                  type="text"
                  name="name3"
              ...
              <div>
                <label className="block text-sm font-bold mb-2">Participant 3 College <span className="text-gray-500">(Optional)</span></label>
                <input
                  type="text"
                  name="college3"
                  value={formData.college3}
                  onChange={handleInputChange}
                  className="w-full bg-gray-900 border border-gray-700 px-4 py-2 text-white focus:border-red-600 focus:outline-none transition"
                  placeholder="Enter participant 3 college"
                />
              </div>
              ...
              >
                {isSubmitting ? 'SUBMITTING...' : 'REGISTER FOR QUIZCORN'}
              </button>
            </form>
          </div>
        </div>
